...
function closed_loop_tracking_composite!(D, state, params, time)
    ...
    if time <= Ts
        ...
        return closed_loop_tracking_backup!(D, state, committed_traj.backup_trajectory, time - Ts)
    end
    return
end
...
function construct_candidate_trajectory(time, state, prob::GatekeeperProblem)
    ...
    for switch_time in range(start=nominal_end_time, stop=time, step=-prob.switch_step_size)
        ...
        backup_path = construct_candidate_backup_trajectory(SVector{3}(nominal_end_state...), prob)
...
#     println("** Update committed called at t=$(time) **")
    ...
    candidate_trajectory = construct_candidate_trajectory(
        time,
        state,
        prob)
...
function simulate_closed_loop_gatekeeper(initial_state, tspan, prob::GatekeeperProblem)
    ...
    first_candidate_trajectory = construct_candidate_trajectory(tspan[1], initial_state, prob)
