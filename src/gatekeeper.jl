...
# Base.show(io::IO, prob::GatekeeperProblem) = print(io, "Gatekeeper Problem(|wezes|: ", length(prob.wezes), ", offset: ", prob.offset, ", turning radius: ", prob.turning_radius, ")")
# Base.show(io::IO, ::MIME"text/plain", prob::GatekeeperProblem) = print(io, "Gatekeeper Problem\n", " - Number of wezes: ", length(prob.wezes), "\n - offset: ", prob.offset, "\n - turning radius: ", prob.turning_radius)
...
#         robot = Wez.Robot(p...)
#         if Wez.is_colliding(wezes, robot)
#             return false
#         end
#     end
#     return true
...
# function construct_reconnection_sites(reference_path::Vector{DubinsPath}, reconnection_step_size=0.01)
    
#     # get a list of reconnections on each segment
#     reconnection_sites = Tuple{Int64, Vector{Float64}}[] # list of [(segment_idx, sample point)]
...
#     reconnection_sites = construct_reconnection_sites(prob.reference_path, prob.reconnection_step_size)
    
#     # find connections paths
#     connection_paths = construct_reconnection_paths(reconnection_sites, initial_state, prob.turning_radius)
...
#         if path_is_collision_free(connection_path, prob.wezes, prob.collision_check_step_size)
...
#             errcode, segment_path = dubins_shortest_path(connection_pt, segment_end_pt, prob.turning_radius)
#             @assert errcode == Dubins.EDUBOK
...
# # simulate tracking the nominal trajectory (without an offset)
# function closed_loop_tracking_backup!(D, state, backup_path::Vector{DubinsPath}, backup_time)
...
#     if time <= Ts
#         return Gatekeeper.closed_loop_tracking_nominal!(D, state, prob, time)
#     else
#         # if after, run a tracking controller on the backup path
#         return Gatekeeper.closed_loop_tracking_backup!(D, state, committed_traj.backup_trajectory, time - Ts)
#     end
#     return
# end
...
#     # create the robot
#     robot = Wez.Robot(state...)
    
#     # get all the wezes
#     wezes = integrator.p.wezes
    
#     # get min distance to a wez
#     return minimum(Wez.collision_distance(wez, robot) for wez in wezes)
...
#     if Wez.is_colliding(prob.wezes, Wez.Robot(state...) )
#         @warn "Robot is already in a wez. Collision distance: $(Wez.collision_distance(prob.wezes, Wez.Robot(state...)))"
#         return nothing
#     end
...
#     odesol = solve(odeproblem, Tsit5(); dt=0.001, dtmax=prob.integration_max_step_size, callback=termination_callback)
...
#     for switch_time in range(start=nominal_end_time, stop=time, step=-prob.switch_step_size)

#         # get the state at the switch time
#         nominal_end_state = nominal_solution(switch_time)
        
#         # construct a backup from this switch time
#         backup_path = construct_candidate_backup_trajectory(nominal_end_state, prob)
...
#     candidate_trajectory = Gatekeeper.construct_candidate_trajectory(
#         time,
#         state,
#         prob)
...
# function simulate_closed_loop_gatekeeper(initial_state, tspan, prob::Gatekeeper.GatekeeperProblem)

#     # first check that the reference path is indeed safe
#     if !Gatekeeper.path_is_collision_free(prob.reference_path, prob.wezes, 0.1*prob.collision_check_step_size)
#         @warn "Reference path is not safe!!"
#     end
    
#     # construct the first committed trajectory
#     first_candidate_trajectory = Gatekeeper.construct_candidate_trajectory(tspan[1], initial_state, prob)
...
#         closed_loop_tracking_composite!, 
#         Vector(initial_state),
#         tspan, 
#         params)
    
#     # update_committed_callback = PeriodicCallback(update_committed_affect!, prob.switch_step_size)
#     update_committed_callback = IterativeCallback(time_choice_gatekeeper, update_committed_affect!)
