...
function Gatekeeper.get_reference_path(gk::GKDubinsWezes2D)
    return gk.reference_path
end

function Gatekeeper.get_offset(gk::GKDubinsWezes2D)
    return gk.offset
end

function Gatekeeper.get_obstacles(gk::GKDubinsWezes2D)::Vector{AbstractWez}
    return gk.wezes
end
...
"""
function Gatekeeper.path_length(gk::GKDubinsWezes2D, p::Dubins.DubinsPath)
    return dubins_path_length(p)
end

function Gatekeeper.path_length(gk::GKDubinsWezes2D, p::Vector{Dubins.DubinsPath})
    len = 0.0
    for path in p
...
function Gatekeeper.get_remaining_nominal(
    ...
    errcode, segment_end_pt =
        dubins_path_endpoint(Gatekeeper.get_reference_path(prob)[path_idx])
    @assert errcode == Dubins.EDUBOK
